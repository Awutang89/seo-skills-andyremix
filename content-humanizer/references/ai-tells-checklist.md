...
## Category 6: Second-order / quantitative tells (survive vocabulary swaps)

First-order tells (Categories 1–5) are words and phrases — a writer can swap them out and still sound
robotic. **Second-order tells are structural and rhythmic**: they survive find-and-replace because they
describe *how* the text is built, not which words it uses. Measure these with an optional
content-metrics helper (tallies burstiness / windowed TTR / structural tics) — or estimate on the densest
section. (Adapted from the `claude-blog` ai-slop-detection model, MIT.)

### Quantitative thresholds (from the helper)
| Metric | What it measures | Flag when | Why / fix |
|---|---|---|---|
| **Sentence-length burstiness** (CV = stdev ÷ mean of sentence word-counts) | Rhythm variation | **< 0.30** 🔴 | Human prose varies sentence length (CV ~0.5–0.7). A metronomic ~0.2 = machine cadence. Fix: add short punches and one long sentence; break the uniformity. |
| **Type-token ratio** (windowed, 500-word segments) | Vocabulary diversity | **< 0.40** 🟡 | Thin/repetitive vocabulary. Use the *windowed* TTR — raw TTR naturally falls on long articles, so never flag a 3,000-word piece on raw TTR. |
| **AI-trigger density** | Category-1 words per 1,000 words | **> 5 / 1k** 🔴 | Same threshold as the Phase 7 gate; the helper lists which triggers hit. |

### The 10 structural tics (binary — present or not)
Each one present is a 🟡; three or more is a 🔴:
1. **Question-heavy headings** — >70% of H2/H3 are questions (some help GEO; all of them is a tell).
2. **"Here" openers** — 3+ sentences/paragraphs starting "Here's…/Here are…".
3. **Metronomic three-clause sentences** — >50% of sentences share the same comma-comma-clause shape.
4. **False-balance framing** — "Some say X, others say Y" with no position taken (we pick a winner).
5. **Hedge clustering** — multiple hedges stacked in one passage (cross-ref Category 2).
6. **Uniform list lengths** — every bulleted list has the same item count / same-length items.
7. **Predictable transitions** — "Moreover, / Furthermore, / Additionally," opening consecutive sections.
8. **Symmetric section lengths** — every H2 ~the same word count (cross-ref Category 3).
9. **Repetitive paragraph openers** — consecutive paragraphs starting the same way ("The…", "This…").
10. **Empty connective sentences** — "Now that we've covered X, let's move to Y" (cross-ref Sentence Contribution Rule).

A draft that passes Categories 1–5 but trips burstiness <0.30 plus 3+ structural tics still reads as AI —
fix the rhythm and structure, not just the vocabulary.
...
**Second-order overlay:** also run the content-metrics helper. Burstiness <0.30, windowed TTR <0.40, or 3+
structural tics (Category 6) each push the verdict one tier toward a fuller rewrite — even if the
Category 1–5 flag count is low. Vocabulary can be clean while the rhythm is still robotic.
